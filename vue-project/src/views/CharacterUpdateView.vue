...
async function updateCharacter() {
  ...
  try {
    ...
    if (response.ok) {
      console.log('Champion modifié')
    } else {
      console.error('Échec de la modification du champion')
    }
  } catch (error) {
  ...
  <div class="add-character-space">
    ...
    <div class="add-character-form">
      ...
      <input
        v-model="description"
        type="text"
        placeholder="Force de noxus"
        class="input"
        id="description"
        required
      />
      <br />
      <label for="aatype" class="label">Nouveau type d'auto-attaque</label>
      <br />
      ...
      <br />
      <div align="center">
        <button @click="updateCharacter" class="submit" name="submit">Modifier le champion</button>
      </div>
    </div>
  </div>
</template>
